...
INITIAL_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    # Open item 7, decided 2026-07-27: the interaction IS gated, against a
    # threshold derived from the Q6 pilot alongside SPEC_MIN_EFFECT and
    # NTA_MIN_DENOMINATOR. The design calls the interaction the signature of a
    # real instrument; gating it is what makes a pass mean that.
    "INTERACTION_MIN_EFFECT": {
        "kind": "constant",
        "declared_value": None,  # Q6 pilot-derived, like the other two
        "consumed_by": ["h1_interaction"],
    },
...
GATE_READS: dict[str, str] = {
    ...
    "h1_interaction": "INTERACTION_MIN_EFFECT",
...
def check_ratification(
    ...
    *,
    ...
    """Enforce separate pilot and confirmatory execution boundaries.

    Pilot mode requires its own authorization but permits pilot-derived constants
    to remain unset because estimating them is the pilot's purpose. Confirmatory
    mode requires the original ratification plus finite, in-range thresholds.
    """
    ...
    if mode == "pilot":
        if thresholds.get("PILOT_AUTHORIZED") is not True:
            raise PreflightError(
                "pilot_not_authorized",
                "PILOT_AUTHORIZED is not set; the pilot requires its own explicit "
                "authorization and cannot borrow confirmatory ratification",
            )
        return

    if thresholds.get("THRESHOLDS_RATIFIED") is not True:
        raise PreflightError(
            "not_ratified",
            "THRESHOLDS_RATIFIED is not set; confirmatory execution requires "
            "Dr. Mani's ratification",
        )
    ...
    for name, entry in (registry or {}).items():
        ...
        if entry.get("kind") == "constant" and value is None:
            raise PreflightError(
                "unset_constant",
                f"{name!r} is still unset while THRESHOLDS_RATIFIED is true",
                constant=name,
            )
        if name not in pilot_thresholds:
            continue
        ...
        if not in_range:
            raise PreflightError(
                "invalid_constant",
                f"{name!r} must be a finite numeric threshold in its valid range, "
                f"got {value!r}",
                constant=name,
                observed=value,
            )
